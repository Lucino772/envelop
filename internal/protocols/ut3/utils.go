...
func getChallengeToken(conn io.ReadWriter, sessionId int32) (int32, error) {
	...
	if err := readHeader(conn, sessionId, 9); err != nil {
		return -1, err
	}
...
func sendQuery(conn io.ReadWriter, sessionId int32, token int32) error {
	...
	return readHeader(conn, sessionId, 0)
}

func readHeader(rd io.Reader, sessionId int32, pktType byte) error {
	var header [5]byte
